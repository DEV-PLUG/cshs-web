...
import MyPetitionDetail from "../../my-detail";
...
export default async function MyPetitionDetailPage({ params }: { params: { id: string } }) {
  return (
        ...
        <div className="flex items-center mb-8 text-sm">
          <Link href='/d/petitions/my'>
            <div className="bg-gray-100 transition-colors cursor-pointer hover:bg-gray-200 px-4 pr-5 py-2 space-x-2 rounded-full text-gray-500 flex items-center">
              ...
              <div>내 청원 목록으로 돌아가기</div>
            </div>
          </Link>
        </div>

        <MyPetitionDetail id={params.id} />
        <MobileBottomMenu />
      </div>
    </main>
  );
